...
public class Token {
    ...
    public static Token delimiterToken(char delimiter){
        switch(delimiter){
            case '{'   : return Token.CURLY_LEFT;
            case '}'   : return Token.CURLY_RIGHT;
            case '('   : return Token.PARENTHESIS_LEFT;
            case ')'   : return Token.PARENTHESIS_RIGHT;
            case '['   : return Token.SQUARE_LEFT;
            case ']'   : return Token.SQUARE_RIGHT;
    ...
    protected Token(char[] tokenChars, int from, int length){
        this.buffer = tokenChars;
        this.from   = from;
        this.length = length;
    }
    ...
    public boolean isString(){
        if(startsWith(Token.QUOTE) && endsWith(Token.QUOTE)) return true;
        if(startsWith(Token.QUOTE_SINGLE)  && endsWith(Token.QUOTE_SINGLE))  return true;
        return false;
    }
